...
export default function OffreStage() {
  ...
  const Submit = async (data) => {
    ...
    if (id == "FnuDTFF8y3Xd542jPI8I") {
      emailjs
        .send(
          "service_x4n11v7",
          "template_iyysomg",
          {
            offre: offreDescription.entreprise,
            email: data.email,
            cv: `${fileURL}}`,
          },
          "user_82DTSux1XbZwguLZSi1bY"
        )
        .then(
          (res) => {
  ...
  return (
    <div>
      ...
      >
          ...
          <div className={styles.offreDescription}>
            ...
            <div className={styles.infos}>
              <div className={styles.info}>
                <h3>
                  <ImLocation className={styles.icon}></ImLocation> Lieu{" "}
                </h3>
                <p>{offreDescription.lieu}</p>
              </div>
              <div className={styles.info}>
                <h3>
                  <RiNewspaperLine className={styles.icon}></RiNewspaperLine>{" "}
                  Contrat{" "}
                </h3>
              ...
              <div className={styles.info}>
                ...
                <a
                  href={offreDescription.website}
                  target="_blank"
                  style={{ color: "#0c97b4" }}
                >
                  {id === "FnuDTFF8y3Xd542jPI8I" ? (
                    <p>cliquez ici </p>
                  ) : (
                    offreDescription.website
                  )}
                </a>
              </div>
              <div className={styles.info}>
                <h3>
                  <VscDebugStart className={styles.icon}></VscDebugStart>Date de
                  début{" "}
                </h3>
              ...
              <div className={styles.info}>
                <h3>
                  <BiTimeFive className={styles.icon} /> Durée{" "}
                </h3>
              ...
              <div className={styles.info}>
                <h3>
                  {" "}
                  <BiCurrentLocation className={styles.icon} /> Adresse{" "}
                </h3>
                <p>{offreDescription.adresse}</p>
              </div>
              <div className={styles.info}>
                <h3>
                  <FaHourglassStart className={styles.icon}></FaHourglassStart>{" "}
                  Postulez avant le{" "}
                </h3>
            ...
            <div className={styles.descriptionContainer}>
              ...
              <div className={styles.description}>
                <div>
                  <p>{offreDescription.description}</p>
                </div>
              </div>
            </div>
            <div className={styles.postuler}>
              {offreDescription.pdf ? (
                <button className={styles.pdf}>
                  <a href={offreDescription.pdf} target="_blank">
                    <BiCloudDownload className={styles.icon}></BiCloudDownload>
                    Télécharger l'offre
                  </a>
                </button>
              ) : null}
              {offreDescription.entreprise == "ICTFICIAL Oy" ? (
        ...
        {open ? (
          <div className={styles.postulerOffre}>
            <div className={styles.filter}>
              ...
              {loader ? (
                ...
                <form onSubmit={handleSubmit(Submit)} className="offreForm">
                  <label>E-mail</label>
                  <input
                    name="email"
                    type="email"
                  ...
                  <button
                    type="submit"
                    className={
                      email.length <= 0 || cv.length <= 0
                        ? null
                        : styles.enabled
                    }
                    disabled={
                      email.length <= 0 || cv.length <= 0 ? true : false
                    }
                  >
                    {" "}
                    Confirmer
